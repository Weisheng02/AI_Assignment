...
| Platform additional functionality | Four custom entities, eight parameters, annotated phrases, required-slot prompting, static official-source responses and fallback handling | Met as an importable artifact; authenticated Dialogflow screenshots remain external |
...
| User feedback and satisfaction | Five-item Google Forms instrument and anonymized verified snapshot in `data/user_feedback_verified.json` | Met descriptively for N=5; small sample prevents generalisation |
...
| Programming | Modular source, atomic JSON writes, validation, confidence handling and 23 automated tests | Met |
...
- Automated checks: 23/23 tests, 10/10 labelled probes for each local client, valid Dialogflow ZIP, and zero Streamlit AppTest exceptions.
...
4. Preserve the anonymous survey snapshot and describe N=5 as preliminary descriptive evidence only; do not claim statistical representativeness.
