...
| Platform additional functionality | Two parameterized fulfillment actions plus tested handler in `src/dialogflow_webhook.py` | Implemented locally; HTTPS deployment and authenticated Dialogflow log remain external |
...
| Programming | Modular source, atomic JSON writes, validation, confidence handling and 27 automated tests | Met |
...
- Automated checks: 27/27 tests, 10/10 labelled probes for each local client, valid Dialogflow ZIP, and zero Streamlit AppTest exceptions.
