...
User-satisfaction results are read only from actual saved submissions in `data/user_feedback.json`. The historical synthetic/unverified entries are retained separately in `data/user_feedback_unverified_sample.json` and are excluded from the report.
...
- Export/anonymise genuine usability responses before reporting survey statistics.
